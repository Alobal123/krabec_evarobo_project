...
class Robot:
    Height = 0.3
    EPS = 0.05
    MATRIX_SHAPE = (4,3,2,4)
    GENOME_LENGTH = 4*3*2*4
        ...
        for i in range(4):
            ...
            for source_id in range(3):
                for target_id in range(2):
                    source = source_id
                    target = target_id
                    start_weight = self.weight_matrix[i,source, target, 0]
                    end_weight = self.weight_matrix[i,source, target, 1]
                    start_time = self.weight_matrix[i,source, target, 2]
                    end_time = self.weight_matrix[i,source, target, 3]
